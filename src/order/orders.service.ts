...
export abstract class ordersService {
  static async listOrdersForBuyer({ buyerId, limit }: { buyerId: string; limit: number }) {
  ...
  static async listOrdersForSeller({ sellerId, limit }: { sellerId: string; limit: number }) {
    const rows = await dbClient
      .select({
        order_id:        schema.orders.id,
        order_status:    schema.orders.status,
  ...
  static async getOrderDetail({ orderId, userId }: { orderId: string; userId: string }) {
    const rows = await dbClient
      .select({
        order_id:        schema.orders.id,
        order_status:    schema.orders.status,
