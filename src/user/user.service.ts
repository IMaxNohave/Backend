...
export abstract class userService {
  // example service nothing actually happening here
  static async getUserById({ id, email }: { id: string; email: string }) {
    return await dbClient.select().from(user).where(eq(user.id, id));
  }
  static async getProfileById({ id }: { id: string }) {
    return await dbClient.select().from(user).where(eq(user.id, id)).limit(1);
  }
  static async updateProfileById({
    ...
    patch.email = email;
    return await dbClient.update(user).set(patch).where(eq(user.id, id));
  }
